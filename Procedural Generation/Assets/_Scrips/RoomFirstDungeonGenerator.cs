...
public class RoomFirstDungeonGenerator : SimpleRandomWalkDungeonGenerator
{
    ...
    private HashSet<Vector2Int> CreateCorridor(Vector2Int currentRoomCenter, Vector2Int distination)
    {
        ...
        {
            if (distination.x > position.x)
            {
                position += Vector2Int.right;
            }else if(distination.x < position.x)
            {
    ...
    private Vector2Int FindClosestPointTo(Vector2Int currentRoomCenter, List<Vector2Int> roomCenters)
    {
        ...
        {
            float currentDistrance = Vector2.Distance(position, currentRoomCenter);
            if (currentDistrance < distance)
            {
                distance = currentDistrance;
                closest = position;
            }
        }
        return closest;
